...
routes = pd.read_csv('../data/combined_routes.csv')
areas = pd.read_csv('../data/combined_areas.csv')
...
selection = st.sidebar.radio('',options = ['Route Recommender','Find Similar Users', 'Find Similar Routes'])


if selection == 'Route Recommender':
	...
	climb_types = st.sidebar.multiselect('Select Climb Type(s)', options = ['Sport', 'Trad', 'Boulder'], default = ['Sport'])
	...
	pitch_options = ['Exactly 1', 'At least 2', 'At least 3', 'At least 4', 'At least 5', '6+ pitches']
	...
	if st.sidebar.button('Get Recommendations'):
		...
		min_pitches = pitch_options.index(pitches)+1
		...
		rec = get_top_n(user_id, n= max_results, area_ids = area_ids,
		  ...
		  max_boulder_grade = max_boulder_grade, pitches = min_pitches, ignore_completed = True)
	...
	if(hit_rec):
		...
		rec2 = rec[['route_name','route_id', 'type',  'grade', 'pitches', 'score', 'location']].reset_index(drop = True)
		...
		for i, row in areas.iterrows():
			if row['parent_id'] == 0:
				...
				folium.Marker([lat,lon], popup = tooltip).add_to(m)
		folium_static(m)
elif selection == 'Find Similar Users':
	st.write('Similar')
elif selection == 'Find Similar Routes':
	route_id = int(st.sidebar.text_input('mountainproject.com route ID:', value = 105918478))
	max_results = int(st.sidebar.number_input(
			'Max Number of recommendations',
			min_value = 1, max_value = 40, step = 1, value = 10,
			key = 'maxresults'))

	if st.sidebar.button('Get Similar Routes'):
		hit_rec = True
		
		rec = get_similar_climbs(route_id, max_results)

	if(hit_rec):
		col1, col2 = st.beta_columns(2)

		st.write(f'Top {max_results} climbs similar to route {route_id}')
